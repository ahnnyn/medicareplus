...
const QuanLyLichHen = () => {
  ...
  const handleStatusChange = async (value, record) => {
    ...
    try {
      const res = await updateThongTinlichKham(
        user.maBacSi,
        record.maLich,
        mappedStatus
      );
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <span>
          <b>{record.hoTen}</b> <br /> SĐT: {record.soDienThoai}
        </span>
      ),
    },
    {
      ...
      render: (_, record) => (
        <span>
          {record.ngayKham} <br /> {record.khungGio}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (_, record) => {
        const { color, label } = getStatusTagThanhToan(
      ...
      render: (_, record) => {
        ...
        return (
          <div style={{ display: "flex", gap: "10px" }}>
            ...
            {/* Nút gọi video */}
            <Tooltip
              title={isDisabled ? "Đã hủy - không thể gọi" : "Gọi video"}
            >
              <BsCameraVideoFill
                style={{
            ...
            {/* Nút nhắn tin */}
            <Tooltip
              title={isDisabled ? "Đã hủy - không thể nhắn tin" : "Nhắn tin"}
            >
          ...
          <Table
            ...
            pagination={{
              ...
              showTotal: (total, range) =>
                `${range[0]}-${range[1]} trong tổng ${total} lịch khám`,
              onChange: (page, size) => {
                setCurrent(page);
                setPageSize(size);
              },
