...
import { fetchLayTTPhieuKhamBenh } from "services/doctor/doctors.services";
import jsPDF from "jspdf";
import html2canvas from "html2canvas";

const ModalHoSoBenhNhan = ({
  openView,
  setOpenView,
  dataView,
  setDataView,
  ...
  const fetchData = async () => {
    try {
      ...
      for (const item of dataArray) {
        const response = await fetchLayTTPhieuKhamBenh(
          item.maLich,
          item.ngayKham,
          item.khungGio
        );
            ...
            return (
              ...
              >
                <div style={{ textAlign: "center", marginBottom: 10 }}>
                  <button
                    onClick={() =>
                      handleDownloadPDF(`printArea-${item.maLich}`)
                    }
                    style={{
                      padding: "6px 12px",
                ...
                <div id={`printArea-${item.maLich}`}>
                  ...
                  >
                    ...
                    <Descriptions.Item label="Thanh toán">
                      {renderTrangThaiThanhToan(item.trangThaiThanhToan)}
                    </Descriptions.Item>
                    <Descriptions.Item label="Giá khám">
                      {parseInt(item.giaKham).toLocaleString()} VNĐ
                    </Descriptions.Item>
                  </Descriptions>
