...
const QuanLyLichHen = () => {
    ...
    const findAllOrder = async () => {
        ...
        if (user) {
            query += `&idDoctor=${encodeURIComponent(user.maBacSi)}`;
        }
        ...
        let res = await findAllLichKhamByBacSi(user.maBacSi);
        console.log("Lịch hẹn bác sĩ: ", res);
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text, record) => (
                <span style={{ fontWeight: "bold" }}>
                    {record?.hoTen} <br /> <span style={{ fontWeight: "500" }}> SĐT:</span> &nbsp;
                    <span style={{ fontWeight: "500" }}>{record?.soDienThoai}</span>
                </span>
            ),
            // width: 100
        },
        {
            ...
            render: (text, record) => {
                return (
                    <>
                        {(record.ngayKham)} {'  '} <br /> {(record.tenGioKham)}
...

        {
            ...
            render: (text, record) => {
                const getStatusTag = () => {
                    ...
                    // if (!record.trangThai && !record.trangThaiKham) {
                    //     return {
                    //         color: "orange",
                    //         icon: <ExclamationCircleOutlined />,
                    //         label: "Chờ xác nhận",
